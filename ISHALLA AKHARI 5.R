...
forecast_and_evaluate <- function(returns, actual_returns) {
  ...
  for (i in 1:100) {
    train_data <- returns[1:(1433 + (5 * i - 5))]
    
    # Fit GARCH
    fit_GARCH <- ugarchfit(spec = spec_GARCH, data = train_data, solver = "hybrid")
    forecast_GARCH <- ugarchforecast(fit_GARCH, n.ahead = 5)
    ...
    forecast_EGARCH <- ugarchforecast(fit_EGARCH, n.ahead = 5)
    ...
    forecast_GAS <- gas_forecast(est_GAS, t_ahead = 5,method = "simulated_paths")
    pred_val_GAS <- forecast_GAS$forecast$y_ahead_sd
  ...
  actual_vol <- tail(returns^2, 500)
  actual_vol <- as.numeric(actual_vol)
  ...
  for (i in 1:500) {
    ...
    cum_rmse_return_garch <- sqrt(cumsum(abs_return_garch^2) )
    cum_rmse_return_egarch <- sqrt(cumsum(abs_return_egarch^2) )
    cum_rmse_return_gas <- sqrt(cumsum(abs_return_gas^2) )
    
    cum_rmse_vol_garch <- sqrt(cumsum(abs_vol_garch^2) )
    cum_rmse_vol_egarch <- sqrt(cumsum(abs_vol_egarch^2))
    cum_rmse_vol_gas <- sqrt(cumsum(abs_vol_gas^2) )
    ...
    dm_return_garch_egarch <- dm.test(error_return_garch, error_return_egarch, h = 5, power = 2)
    dm_return_garch_gas <- dm.test(error_return_garch, error_return_gas, h =5, power = 2)
    dm_return_egarch_gas <- dm.test(error_return_egarch, error_return_gas, h = 5, power = 2)
    
    dm_vol_garch_egarch <- dm.test(error_vol_garch, error_vol_egarch, h = 5, power = 2)
    dm_vol_garch_gas <- dm.test(error_vol_garch, error_vol_gas, h = 5, power = 2)
    dm_vol_egarch_gas <- dm.test(error_vol_egarch, error_vol_gas, h = 5, power = 2)
...
print(results_NG$win_rate_vol_garch)
print(results_NG$win_rate_vol_egarch)
print(results_NG$win_rate_vol_gas)

print(results_WTI$win_rate_vol_garch)
print(results_WTI$win_rate_vol_egarch)
print(results_WTI$win_rate_vol_gas)
...
ylim_range <- range(results_BRENT$pred_vol_EGARCH,results_BRENT$pred_vol_GAS)

plot(results_BRENT$pred_vol_EGARCH, type = "l", col = "blue", lwd = 2, xlab = "Time", ylab = "CSSFED", main = "CSSFED for Different Models WTI 5-steps ", ylim = ylim_range)
lines(results_BRENT$pred_vol_GARCH, col = "red", lwd = 2, lty = 2)
lines(results_BRENT$pred_vol_GAS, col = "green", lwd = 2, lty = 3)
lines(actual_vol, col = "purple", lwd = 2, lty = 3)
abline(h = 0, col = "black", lty = 4) # اضافه کردن خط صفر
returns_BRENT^
  actual_vol <- as.numeric(tail(returns_BRENT^2, 500))

results_BRENT$cum_rmse_vol_garch
results_BRENT$cum_rmse_vol_egarch
results_BRENT$cum_rmse_vol_gas

results_NG$cum_rmse_vol_garch
results_NG$cum_rmse_vol_egarch
results_NG$cum_rmse_vol_gas

results_WTI$cum_rmse_vol_garch
results_WTI$cum_rmse_vol_egarch
results_WTI$cum_rmse_vol_gas
...
cssfed_GAS_GARCH <- results_BRENT$cssfed_return_gas_garch

cssfed_GAS_eGARCH <- results_BRENT$cssfed_vol_gas_egarch
cssfed_GARCH_eGARCH <-results_BRENT$cssfed_vol_garch_egarch
ylim_range <-range(cssfed_GAS_GARCH,cssfed_GAS_eGARCH,cssfed_GARCH_eGARCH) 

plot(cssfed_GAS_GARCH, type = "l", col = "blue", lwd = 2, xlab = "Time", ylab = "CSSFED", main = "CSSFED for Different Models BRENT 60-steps ", ylim = ylim_range)
lines(cssfed_GAS_eGARCH, col = "red", lwd = 2, lty = 2)
lines(cssfed_GARCH_eGARCH, col = "green", lwd = 2, lty = 3)
abline(h = 0, col = "black", lty = 4) # اضافه کردن خط صفر
legend("topleft", legend = c("GAS-GARCH", "GAS-EGARCH", "GARCH-EGARCH"), col = c("blue", "red", "green"), lty = c(1, 2, 3), lwd = 2, cex = 0.5) # کوچک کردن بیشتر لگند

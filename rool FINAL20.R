...
forecast_and_evaluate <- function(returns, actual_returns) {
  ...
  for (i in 1:25) {
    ...
    forecast_EGARCH <- ugarchforecast(fit_EGARCH, n.ahead = 20)
    ...
    forecast_GAS <- gas_forecast(est_GAS, t_ahead = 20,method = "simulated_paths")
    ...
    NORMreturns_train <- returns[(end_train+1) : (end_train + 20)] - train_mean
  ...
  start_idx <- (i-1)*20 + 1
  end_idx <- i*20
  actual_vol[start_idx:end_idx] <- actual_vol1
  ...
  for (i in 1:500) {
    ...
    dm_return_garch_egarch <- dm.test(error_return_garch, error_return_egarch, h = 20, power = 2)
    dm_return_garch_gas <- dm.test(error_return_garch, error_return_gas, h =20, power = 2)
    dm_return_egarch_gas <- dm.test(error_return_egarch, error_return_gas, h = 20, power = 2)
    
    dm_vol_garch_egarch <- dm.test(error_vol_garch, error_vol_egarch, h = 60, power = 2)
    dm_vol_garch_gas <- dm.test(error_vol_garch, error_vol_gas, h = 60, power = 2)
    dm_vol_egarch_gas <- dm.test(error_vol_egarch, error_vol_gas, h = 60, power = 2)
  ...
  list(
    ...
    cssfed_vol_garch_egarch=cssfed_vol_garch_egarch,cssfed_return_gas_garch=cssfed_return_gas_garch,cssfed_return_gas_egarch=cssfed_return_gas_egarch,
...
ylim_range <- range(results_WTI$pred_vol_EGARCH,results_WTI$pred_vol_GAS)

plot(results_WTI$pred_vol_EGARCH, type = "l", col = "blue", lwd = 2, xlab = "Time", ylab = "CSSFED", main = "CSSFED for Different Models WTI 5-steps ", ylim = ylim_range)
lines(results_WTI$pred_vol_GARCH, col = "red", lwd = 2, lty = 2)
lines(results_BRENT$pred_vol_GAS, col = "green", lwd = 2, lty = 3)
lines(results_BRENT$actual_vol, col = "purple", lwd = 2, lty = 3)
...
cssfed_GAS_GARCH <- results_NG$cssfed_return_gas_garch

cssfed_GAS_eGARCH <- results_NG$cssfed_return_gas_egarch
cssfed_GARCH_eGARCH <-results_NG$cssfed_return_garch_egarch
ylim_range <-range(cssfed_GAS_GARCH,cssfed_GAS_eGARCH,cssfed_GARCH_eGARCH) 
plot(cssfed_GAS_GARCH, type = "l", col = "blue", lwd = 2, xlab = "Time", ylab = "CSSFED", 
     main = "CSSFED for return Forecasting (NG,20-Step Ahead)", 
     ylim = ylim_range, cex.main = 0.8) # تغییر اندازه متن عنوان
lines(cssfed_GAS_eGARCH, col = "red", lwd = 2, lty = 2)
lines(cssfed_GARCH_eGARCH, col = "green", lwd = 2, lty = 3)
abline(h = 0, col = "black", lty = 4) # اضافه کردن خط صفر
legend("topleft", legend = c("GAS-GARCH", "GAS-EGARCH", "GARCH-EGARCH"), 
       col = c("blue", "red", "green"), lty = c(1, 2, 3), lwd = 2, cex = 0.4) # کوچک کردن بیشتر لگند
...
results_WTI$win_rate_return_gas
results_WTI$win_rate_return_egarch
results_WTI$win_rate_return_garch

results_NG$win_rate_return_gas
results_NG$win_rate_return_egarch
results_NG$win_rate_return_garch

results_BRENT$win_rate_return_gas
results_BRENT$win_rate_return_egarch
results_BRENT$win_rate_return_garch

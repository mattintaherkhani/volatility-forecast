...
forecast_and_evaluate <- function(returns, actual_returns) {
  ...
  for (i in 1:9) {
    ...
    forecast_EGARCH <- ugarchforecast(fit_EGARCH, n.ahead = 60)
    ...
    forecast_GAS <- gas_forecast(est_GAS, t_ahead = 60,method = "simulated_paths")
    ...
    NORMreturns_train <- returns[(end_train+1) : (end_train + 60)] - train_mean
  ...
  actual_vol <- head(actual_vol, 500)
  ...
  for (i in 1:500) {
    ...
    dm_return_garch_egarch <- dm.test(error_return_garch, error_return_egarch, h = 60, power = 2)
    dm_return_garch_gas <- dm.test(error_return_garch, error_return_gas, h =60, power = 2)
    dm_return_egarch_gas <- dm.test(error_return_egarch, error_return_gas, h = 60, power = 2)
...
cssfed_GARCH_eGARCH <-results_WTI$cssfed_vol_garch_egarch
...
plot(cssfed_GAS_GARCH, type = "l", col = "blue", lwd = 2, xlab = "Time", ylab = "CSSFED", 
     main = "CSSFED for volatility Forecasting (WTI,60-Step Ahead)", 
...
abline(h = 0, col = "black", lty = 4) # اضافه کردن خط صفر
legend("bottomleft", legend = c("GAS-GARCH", "GAS-EGARCH", "GARCH-EGARCH"), 
...
length(results_BRENT$actual_vol)
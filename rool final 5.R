...
forecast_and_evaluate <- function(returns, actual_returns) {
  ...
  for (i in 1:100) {
    ...
    est_GAS <- gas(y = train_data, distr = "t", regress = "sep",
                   optim_arguments = list(opts = list(algorithm = "NLOPT_LN_NELDERMEAD", maxeval = 5000)))
    ...
    NORMreturns_train <- returns[(end_train+1) : (end_train + 5)] - train_mean
...
cssfed_GAS_eGARCH <- results_BRENT$cssfed_return_gas_egarch
cssfed_GARCH_eGARCH <-results_BRENT$cssfed_return_garch_egarch
ylim_range <-range(cssfed_GAS_GARCH,cssfed_GAS_eGARCH,cssfed_GARCH_eGARCH) 

plot(cssfed_GAS_GARCH, type = "l", col = "blue", lwd = 2, xlab = "Observation", ylab = "CSSFED", main = "CSSFED BRENT 5-Step return Forecasts" , ylim = ylim_range)
